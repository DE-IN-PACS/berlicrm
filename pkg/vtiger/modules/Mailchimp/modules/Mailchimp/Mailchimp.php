...
class Mailchimp extends Vtiger_CRMEntity {
	...
	var $table_name = 'vtiger_mailchimp';
	var $table_index= 'mailchimpid';
	 ...
	 */
	var $customFieldTable = Array('vtiger_mailchimpcf', 'mailchimpid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_mailchimp', 'vtiger_mailchimpcf');
	...
	var $tab_name_index = Array(
		...
		'vtiger_mailchimp'   => 'mailchimpid',
	    'vtiger_mailchimpcf' => 'mailchimpid');
	...
	var $list_fields = Array (
		...
		'Mailchimp Name'=> Array('mailchimp', 'mailchimpname'),
		'type' => Array('mailchimp','campaign_type'),
		'status' => Array('mailchimp','campaign_status'),
		'description' => Array('crmentity','description'),
		'Assigned To' => Array('crmentity','smownerid')	
	);
	var $list_fields_name = Array(
		/* Format: Field Label => fieldname */
		'Mailchimp Name'=> 'mailchimpname',
		'type'=>'campaign_type',
	...
	var $list_link_field = 'mailchimpname';
	...
	var $popup_fields = Array('mailchimpname');

	// Placeholder for sort fields - All the fields will be initialized for Sorting through initSortFields
	var $sortby_fields = Array();

	// For Alphabetical search
	var $def_basicsearch_col = 'mailchimpname';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'mailchimpname';

	// Required Information for enabling Import feature
	var $required_fields = Array('mailchimpname'=>1);

	// Callback function list during Importing
	var $special_functions = Array('set_import_assigned_user');

	var $default_order_by = 'mailchimpname';
	...
	function set_import_assigned_user()
	{
        ...
        /**
	 * Function to get Mailchimp Campaign related Contacts
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			//adds sharing access
			$MailChimpModule  = Vtiger_Module::getInstance('Mailchimp');
			Vtiger_Access::setDefaultSharing($MailChimpModule);
			...
			$adb->pquery('INSERT INTO vtiger_settings_field(fieldid, blockid, name, iconpath, description, linkto, sequence)
				VALUES (?,?,?,?,?,?,?)', array($fieldid, $blockid, 'Mail Chimp', 'mailchimp.png', 'LBL_MAILCHIMP_DESCRIPTION',
					'index.php?module=Mailchimp&parent=Settings&view=Index', $seq));

			//Showing Mailchimp module in the related modules at the More Information widget
			$mailchimpInstance = Vtiger_Module::getInstance('Mailchimp');
			$mailchimpLabel = 'Mailchimp';

			$contactsInstance = Vtiger_Module::getInstance('Contacts');
			$contactsInstance->setRelatedlist($mailchimpInstance,$mailchimpLabel,array('SELECT'),'get_related_list');

			$leadsInstance = Vtiger_Module::getInstance('Leads');
			$leadsInstance->setRelatedlist($mailchimpInstance,$mailchimpLabel,array('SELECT'),'get_related_list');
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO vtiger_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("vtiger_modentity_num"), $moduleName, 'MC-', 1, 1, 1));
			}
    ...
    function getMailchimpApiKey(){
		global $adb;
		$sql = "SELECT apikey FROM vtiger_mailchimp_settings WHERE id = 1";
		$result =  $adb->pquery($sql, Array());
		return $result->fields['apikey'];
	}
