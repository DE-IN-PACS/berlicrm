...
function createpdffile ($idnumber,$purpose='', $path='',$current_id='') {
	...
	require_once('modules/SalesOrder/pdf_templates/footer.php');
	...
	global $ACCOUNT_NUMBER, $ROUTING_NUMBER, $SWIFT_NUMBER, $IBAN_NUMBER, $delivery_date;
	...
	include_once("modules/SalesOrder/language/".$pdf_config_details['pdflang'].".lang.pdf.php");
	...
	if ($delivery_date != '') {
		$delivery_date = str_replace ("-",".",getValidDisplayDate($delivery_date));
	}
	...
	if ($purpose=='save') {
		// save PDF file at SO
		$pdf->Output($path.$current_id."_".$export_org.'_'.$pdf_strings['FACTURE1'].'_'.$date_issued.'.pdf','F'); 
		return $export_org.'_'.$pdf_strings['FACTURE1'].'_'.$date_issued.'.pdf';
	}
	// issue pdf
