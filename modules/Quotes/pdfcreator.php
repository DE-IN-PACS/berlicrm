...
function createpdffile($idnumber, $purpose = '', $path = '', $current_id = '')
{
    ...
    require_once('modules/Quotes/pdf_templates/footer.php');
    ...
    include_once("modules/Quotes/language/".$pdf_config_details['pdflang'].".lang.pdf.php");
    ...
    //the default value for currency values is 12345,67 and can get modified by currency settings
    // we need to set default values for customer portal PDF outputs (no user logged in)
    ...
    if (isset($focus->column_fields['requisition_no'])) {
    ...
    $contact_department = $contact_firstname = $contact_lastname =	$contact_salutation = '';
    ...
    if ($final_details['taxtype'] == 'group') {
        ...
        $group_tax_details = empty($final_details['taxes']) ? [] : $final_details['taxes'];
    ...
    //S&H taxes
    $sh_tax_details = empty($final_details['sh_taxes']) ? [] : $final_details['sh_taxes'];
    $sh_tax_percent = '0.00';
