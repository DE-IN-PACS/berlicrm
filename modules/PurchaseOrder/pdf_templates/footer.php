...
class MYPDF extends TCPDF 
{
	...
	public function Footer() 
	{
		...
		global $FOOTER_PAGE, $default_font, $font_size_footer, $PurchaseOrder_no, $NUM_FACTURE_NAME, $pdf_strings, $footer_margin;
		global $org_name, $org_address, $org_city, $org_code, $org_country, $org_state, $org_vatid, $org_taxid, $org_phone, $org_fax, $org_website;
		...
		if ($footerradio =='true') {
			...
			$this->Cell($this->GetStringWidth(decode_html($org_name)),$this->getFontSize(),decode_html($org_name),0,0,'L');
			$this->SetXY(PDF_MARGIN_LEFT , -PDF_MARGIN_FOOTER+12);
			$this->Cell($this->GetStringWidth(decode_html($org_address)),$this->getFontSize(),decode_html($org_address),0,0,'L');
			$this->SetXY(PDF_MARGIN_LEFT , -PDF_MARGIN_FOOTER+16);
			$this->Cell($this->GetStringWidth($org_code." ".decode_html($org_city)),$this->getFontSize(),$org_code." ".decode_html($org_city),0,0,'L');
			$this->SetXY(PDF_MARGIN_LEFT , -PDF_MARGIN_FOOTER+20);
			$this->Cell($this->GetStringWidth(decode_html($org_country)),$this->getFontSize(),decode_html($org_country),0,0,'L');
			//draw line
			...
			$this->Cell($this->GetStringWidth($pdf_strings['VAR_VATID'].' '.$org_vatid),$this->getFontSize(),$pdf_strings['VAR_VATID'].' '.$org_vatid,0,0,'L');
			//draw line
		...
		if ($pageradio =='true') {
			...
			$this->Cell(0,10,$pdf_strings['NUM_FACTURE_NAME'].' '.$PurchaseOrder_no.', '.$pdf_strings['VAR_PAGE'].' '.$this->PageNo().' '.$pdf_strings['VAR_OF'].' '.$this->getAliasNbPages(),0,0,'C');
		}
			//reset colors
			$this->SetTextColor(0,0,0);
	}
