...
class MYPDF extends TCPDF
{
	...
	public function Footer()
	{
		...
		global $org_management, $org_name, $org_address, $org_city, $org_code, $org_country, $org_vatid, $org_state, $org_taxid, $org_phone, $org_fax, $org_website;
		...
		if ($footerradio == 'true') {
			...
			// if management (a.k.a. CEO) is set, we use it ...
			if (!empty($org_management)) {
				$this->Cell($this->GetStringWidth($pdf_strings['MANAGEMENT'] . ' ' . $org_management), $this->getFontSize(), $pdf_strings['MANAGEMENT'] . ' ' . decode_html($org_management), 0, 0, 'L');
			} else {
				// ... else we use the country field
		...
		if ($pageradio == 'true') {
			...
			if ($invoice_status == 'Credit Invoice')
				$this->Cell(0, 10, $pdf_strings['CREDITNUMBER'] . ' ' . $invoice_no . ', ' . $pdf_strings['VAR_PAGE'] . ' ' . $this->PageNo() . ' ' . $pdf_strings['VAR_OF'] . ' ' . $this->getAliasNbPages(), 0, 0, 'C');
			else
				$this->Cell(0, 10, $pdf_strings['NUM_FACTURE_NAME'] . ' ' . $invoice_no . ', ' . $pdf_strings['VAR_PAGE'] . ' ' . $this->PageNo() . ' ' . $pdf_strings['VAR_OF'] . ' ' . $this->getAliasNbPages(), 0, 0, 'C');
		}
		//reset colors
		$this->SetTextColor(0, 0, 0);
	}
