...
function createpdffile($idnumber, $purpose = '', $path = __DIR__ . '/', $current_id = '')
{
	...
	//get account name
	$sql = "select accountname, account_no from  vtiger_account where accountid= ?";
	...
	// using array of pdf globals to assign company values to global variables for later use
	foreach ($pdfGlobals as $key => $value) {
		if ($num_rows > 0) {
			$GLOBALS[$key] = $adb->query_result($result, 0, $value);
			if ($value == "logoname") {
				$GLOBALS[$key] = decode_html($GLOBALS[$key]);
			}
		} else {
	...
	if ($eInvoice) {
		...
		$document
			...
			->setDocumentSeller($org_name, "549910")
			...
			->setDocumentSellerAddress($org_address, "", "", $org_code, $org_city, $org_country)
