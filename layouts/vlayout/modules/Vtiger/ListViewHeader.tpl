    ...
    <div class="listViewTopMenuDiv noprint">
        <div class="listViewActionsDiv row-fluid">
            <span class="btn-toolbar col-md-4">
                <span class="btn-group listViewMassActions">
                    {if count($LISTVIEW_MASSACTIONS) gt 0 || $LISTVIEW_LINKS['LISTVIEW']|@count gt 0}
                        <button class="btn btn-light dropdown-toggle" data-toggle="dropdown"><strong>{vtranslate('LBL_ACTIONS', $MODULE)}</strong>&nbsp;&nbsp;<i class="bi bi-caret-down-fill"></i></button>
                            ...
                            {foreach item="LISTVIEW_MASSACTION" from=$LISTVIEW_MASSACTIONS name=actionCount}
                                <li id="{$MODULE}_listView_massAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_MASSACTION->getLabel())}">
                                    <a href="javascript:void(0);" {if stripos($LISTVIEW_MASSACTION->getUrl(), 'javascript:')===0}onclick='{$LISTVIEW_MASSACTION->getUrl()|substr:strlen("javascript:")};'{else} onclick="Vtiger_List_Js.triggerMassAction('{$LISTVIEW_MASSACTION->getUrl()}')"{/if}>
                                        {vtranslate($LISTVIEW_MASSACTION->getLabel(), $MODULE)}
                                    </a>
                                </li>
                ...
                {foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
                    <span class="btn-group">
                        <button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn btn-light addButton" {if stripos($LISTVIEW_BASICACTION->getUrl(), 'javascript:')===0} onclick='{$LISTVIEW_BASICACTION->getUrl()|substr:strlen("javascript:")};'{else} onclick='window.location.href="{$LISTVIEW_BASICACTION->getUrl()}"'{/if}>
                            <i class="bi bi-plus"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong>
                        </button>
                    </span>
                {/foreach}
            </span>
                    ...
                    {if $CUSTOM_VIEWS|@count gt 0}
                        ...
                        <span class="filterActionsDiv hide">
                            ...
                            <ul class="filterActions">
                                <li data-value="create" id="createFilter" data-createurl="{$CUSTOM_VIEW->getCreateUrl()}"><i class="bi bi-plus-circle"></i> {vtranslate('LBL_CREATE_NEW_FILTER')}</li>
                            </ul>
                        </span>
            ...
            <span class="hide filterActionImages pull-right">
                <i title="{vtranslate('LBL_DENY', $MODULE)}" data-value="deny" class="bi bi-x-circle-fill align-middle denyFilter filterActionImage"></i>
                <i title="{vtranslate('LBL_APPROVE', $MODULE)}" data-value="approve" class="bi bi-check-circle-fill align-middle approveFilter filterActionImage"></i>
                <i title="{vtranslate('LBL_DELETE', $MODULE)}" data-value="delete" class="bi bi-trash-fill align-middle deleteFilter filterActionImage"></i>
                <i title="{vtranslate('LBL_EDIT', $MODULE)}" data-value="edit" class="bi bi-pencil-fill align-middle editFilter filterActionImage"></i>
            </span>
