...
{strip}
	<form name="EditWorkflow" action="index.php" method="post" id="workflow_step2" class="form-horizontal" >
		...
		<input type="hidden" id="olderConditions" value='{json_encode($WORKFLOW_MODEL->get('conditions'))}' />
		...
		<input type="hidden" name="schdayofweek" value={json_encode($WORKFLOW_MODEL->get('schdayofweek'))} />
		<input type="hidden" name="schdayofmonth" value={json_encode($WORKFLOW_MODEL->get('schdayofmonth'))} />
		<input type="hidden" name="schannualdates" value={json_encode($WORKFLOW_MODEL->get('schannualdates'))} />
